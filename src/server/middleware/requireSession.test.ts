...
describe('requireSession', () => {
  ...
  it('responds 401 when session exists but isSessionValid returns false', () => {
    mockGetSession.mockReturnValue({ cookies: [], personId: '', portfolioId: '', valuation: '', authenticatedAt: 0, expiresAt: 0 });
  ...
  it('calls next() when session is valid', () => {
    mockGetSession.mockReturnValue({ cookies: [], personId: '', portfolioId: '', valuation: '', authenticatedAt: 0, expiresAt: Date.now() + 9999 });
